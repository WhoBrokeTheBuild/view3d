...
void print_versions()
{
  ...
  DEBUG_INFO("GLMM Version: %s", GLMM_VER_STRING);
  DEBUG_INFO("LibMDL Version: %s", MDL_VER_STRING);
  DEBUG_INFO("OpenGL Version: %s", glGetString(GL_VERSION));
  DEBUG_INFO("(Free)GLUT Version: %d.%d.%d", glut_maj, glut_min, glut_pat);
  DEBUG_INFO("GLEW Version: %d.%d.%d", GLEW_VERSION_MAJOR, GLEW_VERSION_MINOR,
             GLEW_VERSION_MICRO);
}
...
int main(int argc, char **argv)
{
  ...
  shader_info_t shaders[] = {{GL_VERTEX_SHADER, "shaders/default.vs.glsl"},
                             {GL_FRAGMENT_SHADER, "shaders/default.fs.glsl"},
                             {0, NULL}};

  GLuint def_shader = shader_program_load(shaders);

  shader_data_t data = {(void *)&g_render_data, &render_data_init_cb,
                        &render_data_bind_cb};
